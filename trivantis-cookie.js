...
function saveVariable(name,value,days,title,lms, bHidden) {
  ...
  if( typeof(Storage)!=="undefined" && window.localStorage ){
	...
	var key = "Lectora" + ( title ? (":"+ Encode(title)) : "" );
	var data
	...
	if( data ){
		...
		for( var i = 0; i<dataSplit.length; i++ ){
			...
			if( varParts.length == 2 ){
				...
				newData += (escapeDelim(n) + '=' + escapeDelim(v) + ',')
			}
		}
	}
	
	if( !bFound ){
		newData += (escapeDelim(name) + '=' + escapeDelim(value) + ',')
	}
...
function saveVariableInCookie( name,value,days,title,lms, bHidden ){
  ...
  for ( var i=0; i<ca.length; i++ )
  {
    ...
    if ( c.indexOf(myCookie) === 0 )
    {
      ...
      if ( varIdx >= 0 )
      {
        ...
        lastPart = lastPart.substring(lastPart.indexOf(',')) // , or ,n3=v3,
        lastPart = ( !firstPart && lastPart == ',' ? '' : lastPart )
  ...
  var newVal = ( isDel ? '' : nameEQ + encValue + ',' );
...
{
	s = s.toString();
	return s.replace(/%/g, "%25").replace(/=/g, "%3D").replace(/,/g, "%2C");
}
function unescapeDelim( s )
{
	s = s.toString();
	return s.replace(/%2C/g, ",").replace(/%3D/g, "=").replace(/%25/g, "%");
}
function convertCookies( title, bHidden )
{
	...
	for ( var baseName in oldCookies )
	{
		...
		if( typeof(Storage) !==  "undefined" && window.localStorage)
		{
			var keyName =  "Lectora:" + Encode(title)
			var data = ""
			for ( i = 0; i < vars.length; i++ )
			{
				...
				if( oldParts.length == 2 )
				{
					...
					var newNvPair = newName + '=' + newValue + ','
					data += newNvPair;
				}
		...
		else
		{
			...
			if ( ncv.length > 1 )
			{
				document.cookie = '~' + baseName + nci + '=' + ncv + (isPerm ? expires : '') + '; path=/'
...
function VarUpdateValue() {
  ...
  if( this.cm ) {
    ...
    var titleMgr = getTitleMgrHandle();
    if( titleMgr ) {
      var vv=new String(titleMgr.getVariable(this.name,this.defVal,this.exp));
      this.value=vv;
    }
    ...
    if( this.origAICC ) {
      ...
      if( this.bSCORM ) {
        ...
        if( titleMgr && titleMgr.findVariable( this.name ) != -1 ){
          ...
          var data=new String( GetSuspendData() )
    ...
    if( this.bSCORM ) {
      ...
      if( this.name == 'cmi.core.total_time' || this.name == 'cmi.total_time' ) {
        ...
        if( this.aiccgroup ) {
          ...
          var chkdata=new String( GetSuspendData() )
